...
class CardScrollingActivity : AppCompatActivity(), SensorEventListener, LocationListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val button : Button = findViewById<Button>(R.id.view_cards_button)
    ...
    override fun onLocationChanged(location: Location) {
        val userInfoContainer = UserInfoContainer(location, null, loggedInUser?.token)
        val builder: Retrofit.Builder = Retrofit.Builder().baseUrl("http://appsec.moyix.net").addConverterFactory(
            GsonConverterFactory.create())
        val retrofit: Retrofit = builder.build()
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        if (event != null) {
            val userInfoContainer = UserInfoContainer(null, event.values[0].toString(), loggedInUser?.token)
            val builder: Retrofit.Builder = Retrofit.Builder().baseUrl("http://appsec.moyix.net").addConverterFactory(
                GsonConverterFactory.create())
            val retrofit: Retrofit = builder.build()
            val client: UserInfo = retrofit.create(UserInfo::class.java)
